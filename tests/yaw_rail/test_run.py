...
def write_expect_wsp(path: Path) -> Path:
    ...
    with open(target, "w") as f:
        f.write(
            """# correlation function estimate with symmetric 68% percentile confidence
#    z_low     z_high         nz     nz_err
 0.2002429  0.9998286  0.0107186  0.0033967
 0.9998286  1.7994142  0.0109233  0.0049397
"""
        )
    return target
...
#    z_low     z_high         nz     nz_err
 0.2002429  0.9998286  0.1249581  0.0238401
 0.9998286  1.7994142  0.0912220  0.0158275
"""
        )
    return target
...
def test_run(tmp_path, mock_data, mock_rand, zlim) -> None:
    ...
    w_ss = YawAutoCorrelate.make_stage(name="w_ss", **corr_config).correlate(cache_ref)
    w_ss.data.sample().to_files(tmp_path / "wss")
    assert_cols_match(write_expect_wss(tmp_path), tmp_path / "wss.dat", ignore_cols=[3])

    w_sp = YawCrossCorrelate.make_stage(name="w_sp", **corr_config).correlate(
        cache_ref, cache_unk
    )
